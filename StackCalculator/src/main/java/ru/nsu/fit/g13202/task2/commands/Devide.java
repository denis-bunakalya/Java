...
import ru.nsu.fit.g13202.task2.model.ExceptionArithmeticDivisionByZero;
...
public class Devide implements Command {
	...
	private static Logger LOG = Logger.getLogger(Devide.class.getName());
	...
	public void execute(List<String> arguments, Context context)
		...
		if (second == 0) {
			...
			String s = "Error: division by zero, "
					+ "for the command \"/\" the second element in the stack can't be zero";
			LOG.log(Level.WARNING, s);
			throw new ExceptionArithmeticDivisionByZero(s);
		}
